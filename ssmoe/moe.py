...
class LoRA_MOE_LM(nn.Module): # for llm
    ...
    def __init__(
        ...
        self.n_clusters = getattr(args, 'n_clusters', num_experts)
        self.cluster_prior_weight = float(getattr(args, 'cluster_prior_weight', 1.0))
        self.router_tau = float(getattr(args, 'router_tau', 1.0))
        self._cluster_ids_current = None  # set externally per batch
        ...
        # cluster-conditioned prior over experts (logits)
        self.cluster_prior = nn.Embedding(self.n_clusters, self.num_experts)
        nn.init.zeros_(self.cluster_prior.weight)

    def set_cluster_ids(self, cluster_ids: torch.Tensor):
        """Attach current batch cluster ids: LongTensor [batch]. Use -1 for unknown."""
        self._cluster_ids_current = cluster_ids
    ...
    def forward(self, x):
        ...
        # cluster-conditioned prior (broadcast across sequence)
        if self._cluster_ids_current is not None:
            cids = self._cluster_ids_current
            if not torch.is_tensor(cids):
                cids = torch.as_tensor(cids, device=x.device)
            cids = cids.to(device=x.device)
            # build prior logits [batch, num_experts]
            prior = torch.zeros((x.size(0), self.num_experts), device=x.device, dtype=logits.dtype)
            valid = (cids >= 0) & (cids < self.n_clusters)
            if valid.any():
                prior[valid] = self.cluster_prior(cids[valid])
            # expand to [batch, seq_len, num_experts]
            prior = prior[:, None, :]
            logits = logits + self.cluster_prior_weight * prior

        if self.router_tau != 1.0:
            logits = logits / self.router_tau
